...
<template>
  ...
  <section v-else-if="servicio" class="detalle-servicio">
    <div class="detalle-servicio__visual tarjeta-catalogo--rosa">
      <img v-if="servicio.imagen_ruta" :src="obtenerUrlImagen(servicio.imagen_ruta)" :alt="servicio.nombre" decoding="async" />
    ...
    <div class="detalle-servicio__contenido">
      ...
      <dl class="datos-servicio"><div><dt>Duración</dt><dd>{{ servicio.duracion_minutos }} minutos</dd></div><div><dt>Valor</dt><dd>{{ formatearPrecioServicio(servicio.precio_desde, servicio.precio_hasta) }}</dd></div></dl>
